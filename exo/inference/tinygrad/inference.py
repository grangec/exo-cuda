...
Tensor.no_grad = True
# default settings
TEMPERATURE = float(os.getenv("TEMPERATURE", 0.85))  # must be float, not int: int(0.85) = 0
TOP_K = 25
TOP_P = 0.9
ALPHA_F = 0.1
ALPHA_P = 0.0
...
def build_transformer(model_path: Path, shard: Shard, model_size="8B", device=None):
  ...
  weights = convert_from_huggingface(weights, model, MODEL_PARAMS[model_size]["args"]["n_heads"], MODEL_PARAMS[model_size]["args"]["n_kv_heads"], shard)
...
class TinygradDynamicShardInferenceEngine(InferenceEngine):
  ...
  async def ensure_shard(self, shard: Shard):
    ...
    async with self._load_lock:
      ...
      if self.shard != shard:
        # Free old model before building new one to avoid double VRAM allocation
        self.model = None
        self.shard = None
        import gc
        gc.collect()

        # tinygrad LRU allocator caches freed GPU buffers; flush to reclaim VRAM
